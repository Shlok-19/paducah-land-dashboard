...
let selected=null,session=null,basemap='sat',view={z:16,lon:-88.734,lat:37.116},drag=null;
...
async function syncSession(){const {data}=await sb.auth.getSession();session=data.session;updateAuth()}
function updateAuth(){
 const b=$('authBtn');
 if(session){b.textContent='Sign Out';b.title=session.user?.email||'Editor signed in'}else{b.textContent='Editor Login';b.title='Sign in to edit'}
 if(selected)applyEditState();
}
async function authAction(){
 if(session){await sb.auth.signOut();session=null;updateAuth();return}
 const email=prompt('Enter your authorized editor email. A secure sign-in link will be sent to that address.');
 if(!email)return;
 const {error}=await sb.auth.signInWithOtp({email,options:{shouldCreateUser:false,emailRedirectTo:location.href}});
 if(error)alert('Sign-in could not be started: '+error.message+'\n\nCreate/invite this editor in Supabase Authentication first.');
 else alert('Check '+email+' for the Supabase sign-in link.');
}
...
function applyEditState(){const editable=!!session;['fAcres','fOwner','fStatus','fPrice','fCurrent','fNext','fNotes','fUpdated'].forEach(id=>$(id).disabled=!editable);$('save').disabled=!editable;$('save').textContent=editable?'Save to Cloud':'Login to Edit';$('editNote').textContent=editable?'Editor mode — changes save to the shared cloud database.':'Public view — sign in as an editor to make shared changes.'}
function openParcel(id,zoomTo){const p=parcels.find(x=>x.id===id);if(!p)return;selected=id;$('drawerTitle').textContent=p.owner;$('drawerSub').textContent=fmt(p.acres)+' acres';$('fAcres').value=p.acres;$('fOwner').value=p.owner;$('fStatus').value=p.status;$('fPrice').value=p.pricePerAcre;$('fCurrent').value=p.currentPosition;$('fNext').value=p.nextAction;$('fNotes').value=p.notes;$('fUpdated').value=p.lastUpdated;$('drawer').classList.add('open');applyEditState();if(zoomTo){const c=centroid(p.coords);view.lon=c[0];view.lat=c[1];view.z=Math.max(view.z,18);renderMap()}}
function closeDrawer(){$('drawer').classList.remove('open');selected=null}
async function saveEdit(){if(!session){await authAction();return}const p=parcels.find(x=>x.id===selected);if(!p)return;const payload={owner:$('fOwner').value.trim(),acres:Number($('fAcres').value||0),status:$('fStatus').value,price_per_acre:$('fPrice').value===''?null:Number($('fPrice').value),current_position:$('fCurrent').value.trim()||null,next_action:$('fNext').value.trim()||null,notes:$('fNotes').value.trim()||null,last_updated:$('fUpdated').value||new Date().toISOString().slice(0,10),updated_at:new Date().toISOString()};$('save').disabled=true;$('save').textContent='Saving…';const {error}=await sb.from('parcels').update(payload).eq('id',p.id);if(error){alert('Cloud save failed: '+error.message);applyEditState();return}await loadCloud(true);closeDrawer();setCloud('Cloud: saved','ok')}
...
map.addEventListener('mousedown',e=>{drag={x:e.clientX,y:e.clientY,c:centerWorld()};map.classList.add('drag')});window.addEventListener('mousemove',e=>{if(!drag)return;const ll=unworld(drag.c[0]-(e.clientX-drag.x),drag.c[1]-(e.clientY-drag.y),view.z);view.lon=ll[0];view.lat=ll[1];renderMap()});window.addEventListener('mouseup',()=>{drag=null;map.classList.remove('drag')});map.addEventListener('wheel',e=>{e.preventDefault();view.z=Math.max(8,Math.min(20,view.z+(e.deltaY<0?1:-1)));renderMap()},{passive:false});
$('fit').onclick=fitSite;$('fit2').onclick=fitSite;$('zin').onclick=()=>{view.z=Math.min(20,view.z+1);renderMap()};$('zout').onclick=()=>{view.z=Math.max(8,view.z-1);renderMap()};$('base').onclick=()=>{basemap=basemap==='sat'?'street':'sat';$('base').textContent=basemap==='sat'?'Street Map':'Satellite';renderTiles()};$('search').oninput=refresh;$('statusFilter').onchange=refresh;$('ownerFilter').onchange=refresh;$('close').onclick=closeDrawer;$('cancel').onclick=closeDrawer;$('save').onclick=saveEdit;$('csv').onclick=exportCSV;$('json').onclick=()=>download('Paducah_Land_Tracker_Backup.json',JSON.stringify(parcels,null,2),'application/json');$('authBtn').onclick=authAction;$('refreshCloud').onclick=()=>loadCloud();window.addEventListener('resize',renderMap);
sb.auth.onAuthStateChange((_event,newSession)=>{session=newSession;updateAuth();if(newSession)loadCloud(true)});
refresh();syncSession();loadCloud().then(()=>setTimeout(fitSite,80));setInterval(()=>loadCloud(true),15000);
